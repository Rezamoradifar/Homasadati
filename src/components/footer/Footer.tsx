...
function BadgeRow({items, titleKey}: {items: readonly FooterBadge[]; titleKey: string}) {
  ...
  return <section aria-label={t(titleKey)} className="space-y-3">
    <h2 className="text-xs font-medium uppercase tracking-wider text-[#B9C6D5]">{t(titleKey)}</h2>
    <ul className="flex flex-wrap gap-3">{items.map(item => {
      ...
      const cls = 'inline-flex min-h-11 items-center gap-2 rounded-lg border border-white/20 px-3 py-2 text-xs text-[#E7EAF0]';
      return <li key={item.id}>{item.href ? <a href={item.href} className={`${cls} hover:border-[#DCC38A] ${focusRing}`}>{body}</a> : <span className={cls}>{body}</span>}</li>;
    })}</ul>
  </section>;
}
...
export function Footer({id, className = '', direction, logo, homeHref, resolveHref = localizedHref,
  ...
  const inputClass = `min-h-12 rounded-lg border border-white/25 bg-[#22334D] px-3 text-sm text-white ${focusRing}`;
    ...
    <motion.div initial={false} whileInView={reducedMotion ? undefined : {y: [12, 0], opacity: [0.8, 1]}}
      ...
      <section aria-labelledby={`${uid}-newsletter`} className="grid gap-6 border-b border-white/15 pb-9 lg:grid-cols-2 lg:items-center lg:gap-16">
        ...
        <form onSubmit={subscribe} noValidate aria-label={t('newsletter.title')} aria-busy={status === 'pending'}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="relative min-w-0 flex-1"><Mail aria-hidden="true" size={18} className="pointer-events-none absolute start-4 top-4 text-[#B9C6D5]" />
            ...
            <button disabled={!onSubscribe || status === 'pending'} type="submit" className={`flex min-h-12 shrink-0 items-center justify-center gap-3 rounded-lg bg-[#DCC38A] px-5 text-sm font-semibold text-[#18263D] transition-colors hover:bg-[#F0DBAE] disabled:cursor-not-allowed disabled:opacity-60 ${focusRing}`}>
              {t(status === 'pending' ? 'newsletter.pending' : 'newsletter.submit')}<ArrowUpRight aria-hidden="true" size={17} className="rtl:-scale-x-100" />
            </button>
          </div>
          <p id={`${uid}-notice`} className="mt-3 text-xs leading-5 text-[#B9C6D5]">{t(onSubscribe ? 'newsletter.notice' : 'newsletter.unavailable')}</p>
      ...
      <div className="grid gap-x-6 py-8 md:grid-cols-5 md:gap-x-5 md:py-12 lg:gap-x-10">
        <FooterColumn titleKey="brand.name" defaultOpen>
          ...
          <p className="mt-4 max-w-xs text-sm leading-7 text-[#B9C6D5]">{t('brand.tagline')}</p>
          {socials.length > 0 && <ul aria-label={t('aria.socials')} className="mt-5 flex flex-wrap gap-2">{socials.map(social => <li key={social.id}>
            <a href={social.href} aria-label={t(social.labelKey)} className={`flex size-11 items-center justify-center rounded-full border border-white/20 text-[#D5DCE5] transition-colors hover:border-[#DCC38A] hover:text-[#DCC38A] ${focusRing}`}><span aria-hidden="true">{social.icon}</span></a>
          </li>)}</ul>}
        </FooterColumn>
      ...
      <div className="flex flex-col gap-5 border-t border-white/15 py-6 md:flex-row md:items-end md:justify-between">
        ...
        <div>
          <div className="flex flex-wrap gap-4" aria-busy={switchStatus === 'pending'}>
            <label className="flex min-w-0 flex-1 flex-col gap-2 text-xs text-[#B9C6D5]" htmlFor={`${uid}-language`}>{t('switchers.language')}
            ...
            <label className="flex min-w-0 flex-1 flex-col gap-2 text-xs text-[#B9C6D5]" htmlFor={`${uid}-currency`}>{t('switchers.currency')}
