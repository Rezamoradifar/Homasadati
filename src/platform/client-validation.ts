...
import {merchantContractSchema,merchantProductSchema,merchantPaymentSchema,merchantFulfillmentSchema} from "./merchant-model";
import {merchantSchema, pointsAdjustmentSchema, rewardSchema, redeemSchema, redemptionReviewSchema} from "./club-model";
import {
  travelRuleSchema,
...
export function validateClient(path: string, method: string, data: unknown) {
  ...
  if(path==='admin/binary-rules')schema=z.object({rules:binaryRulesSchema,reason:text});
  if(path==='admin/binary-simulate')schema=simulationSchema;
  if(path==='admin/loyalty-policy')schema=z.object({policy:loyaltyPolicySchema,reason:text});
  ...
  if(path==='admin/merchant-operations/products')schema=merchantProductSchema;
